...
==========================================================================*/
BOOL CAdoRecordSet::Update(CString& sErrorInfo)
{
	ASSERT(m_pRecordset != NULL);
	try
	{
	...
	{
		sErrorInfo.Format("%s",e.ErrorMessage());
...
BOOL CAdoRecordSet::GetChunk(LPCSTR strFieldName, HBITMAP& hBmp)//CBitmap &bitmap)
{		
	BOOL bret = FALSE;
	...
	if ((adFldLong & GetFieldAttributes(strFieldName)) && size > 0)
	{
		...
		if (GetChunk(GetField(strFieldName), (LPVOID)lpData))
		{
			...
			if (bmpHeader.bfType == (*(WORD*)"BM"))
			{
				...
				HDC hdc = GetDC(NULL);
				dc.Attach(hdc);
				hBmp = CreateDIBitmap(dc.m_hDC, &bmpiHeader, CBM_INIT, lpDIBBits, &bmpInfo, DIB_RGB_COLORS);
